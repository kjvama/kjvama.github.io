/*
 * Markdown Renderer + Table + Full Table functionality.
 *
 * This file loads a Markdown document specified by the
 * data-markdown attribute of this script element and renders
 * the Markdown content into the #markdown-content element.
 *
 * It also provides table enhancement and Full Table functionality.
 *
 * The Modal ID is provided by the data-modal attribute of this
 * script element. The renderer does not define or hard-code
 * the Modal ID.
 *
 * The Modal module is used when Full Table functionality is invoked.
 * 
 ...
 * 
 * Example:
 * <script src="/js/markdown-renderer-extended.js"
 *         data-markdown="???.md"
 *         data-modal="fullTableModal"
 *         data-table-container="fullTableContainer"
 *         data-toc-headings="h2,h3,h4"></script>
 * <script src="/js/modal.js"></script>
 *
 */
let modalId = null;
let tableContainerId = null;
let tocHeadings = "h2";
async function loadMarkdown() {
    ...
    modalId = script.dataset.modal;
    tableContainerId = script.dataset.tableContainer;
...
function enhanceKjvamaTables() {
  const tables = [...content.querySelectorAll("table")];

  // The first table is the document metadata table.
  // The second table is the Documents table used by View Full Table.
  const documentsTable = tables[1];
  if (!documentsTable) return;

  // Use the existing empty KJVAMA scroll placeholder from the MD.
  const existingScroll = [...content.querySelectorAll(".kjvama-table-scroll")]
    .find(el => !el.querySelector("table"));

  const tableScroll = documentsTable.closest(".kjvama-table-scroll") || (() => {
    const wrapper = document.createElement("div");
    wrapper.className = "kjvama-table-scroll";
    documentsTable.parentNode.insertBefore(wrapper, documentsTable);
    wrapper.appendChild(documentsTable);
    return wrapper;
  })();

  if (!tableScroll.contains(documentsTable)) {
    tableScroll.appendChild(documentsTable);
  }

  // Create a top scrollbar, matching the KJVAMA screen where the horizontal
  // scrollbar is visible above the table.
  const topScroll = document.createElement("div");
  topScroll.className = "kjvama-table-scroll kjvama-table-scroll--top";
  const topSizer = document.createElement("div");
  topSizer.className = "kjvama-table-scroll__sizer";
  topScroll.appendChild(topSizer);

  tableScroll.parentNode.insertBefore(topScroll, tableScroll);

  function syncScrollSize() {
    topSizer.style.width = documentsTable.scrollWidth + "px";
  }
  ...
  window.addEventListener("resize", syncScrollSize);
  requestAnimationFrame(syncScrollSize);

  // Bind the MD's existing View Full Table button to the Documents table.
  const button = content.querySelector(".kjvama-table-view-full");
  if (button) {
    button.addEventListener("click", () => {
      ...
      modalScroll.className = "kjvama-table-scroll kjvama-table-scroll--modal";

      const clone = documentsTable.cloneNode(true);
      modalScroll.appendChild(clone);
      container.appendChild(modalScroll);

      KjvamaModal.open(modalId);
    });
  }

  // Prevent the unused MD placeholder from becoming a second visible block.
  if (existingScroll && existingScroll !== tableScroll) {
    existingScroll.remove();
  }

  // Keep metadata tables readable without adding document-register behavior.
  tables.filter(t => t !== documentsTable).forEach(table => {
    if (!table.closest(".kjvama-table-scroll")) {
      const wrapper = document.createElement("div");
      wrapper.className = "kjvama-table-scroll kjvama-table-scroll--metadata";
